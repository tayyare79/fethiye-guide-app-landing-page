...
This repository also contains a small Cloudflare Worker for daily Fethiye event imports, scheduled duty-pharmacy snapshots, and the public iOS app API.
...
curl "http://127.0.0.1:8787/api/duty-pharmacies?city=fethiye"
curl "http://127.0.0.1:8787/api/health"
```

### Cron import

`wrangler.jsonc` defines a daily event Cron Trigger and a 30-minute duty-pharmacy Cron Trigger:
...
Cloudflare runs Cron Triggers in UTC. The Worker calls `scheduled()` and imports enabled public event sources into D1 only on the daily `15 4 * * *` trigger. Duty pharmacies are scraped on every trigger, persisted as last-good snapshots, and served from D1.
...
- `PHARMACY_IMPORT_USER_AGENT`: crawler user-agent sent to duty-pharmacy sources.
...
Duty-pharmacy clients can use:

```text
GET https://fethiye-app.com/api/duty-pharmacies?city=fethiye
GET https://fethiye-app.com/api/duty-pharmacies?city=bodrum
GET https://fethiye-app.com/api/duty-pharmacies?city=marmaris
```

See `docs/duty-pharmacies-api.md` for the response contract, stale-data behavior, schedule, storage, and fourth-city instructions.
